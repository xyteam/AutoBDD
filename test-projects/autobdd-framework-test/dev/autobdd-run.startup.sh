...
if [ "$USER" != "root" ]; then
    ...
    # DEV MODE: if AUTOBDD_DEV_MOUNT=1, the working-tree AutoBDD (incl. its
    # node_modules) is already mounted at $HOME/Projects/AutoBDD — do NOT
    # overwrite it with the image's baked copy (which is wdio 6 / stale).
    if [ "$AUTOBDD_DEV_MOUNT" != "1" ]; then
