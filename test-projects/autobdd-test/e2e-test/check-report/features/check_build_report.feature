...
Feature: test build report
  ...
  Scenario: download drivers
    ...
    And   I expect that the element "div.total" inside the 1st parent element "table.chart" does contain the text "4"
    And   I expect that the 1st element "table.tile_info" does match the regex "Passed[\s]+100.00 %[\s]+Failed[\s]+0.00 %"
    And   I expect that the element "div.total" inside the 2nd parent element "table.chart" does contain the text "6"
